#!/usr/bin/env bash
# codebase-mapping (onboarding map) — new-map.sh bootstrap behaviour.
source "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/lib.sh"

test_new_map_requires_slug() {
  local proj; proj=$(make_tmp_project)
  install_kit_into "$proj"
  if ( cd "$proj" && bash talaka/skills/codebase-mapping/new-map.sh ) >/dev/null 2>&1; then
...
test_new_map_creates_folder() {
  local proj; proj=$(make_tmp_project)
  install_kit_into "$proj"
  ( cd "$proj" && bash talaka/skills/codebase-mapping/new-map.sh payments ) >/dev/null
...
test_new_map_is_idempotent() {
  ...
  local out
  out=$( cd "$proj" && bash talaka/skills/codebase-mapping/new-map.sh payments )
...
test_new_map_emits_map_path() {
  ...
  install_kit_into "$proj"
  local out
  out=$( cd "$proj" && bash talaka/skills/codebase-mapping/new-map.sh auth )
