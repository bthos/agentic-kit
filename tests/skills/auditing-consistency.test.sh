#!/usr/bin/env bash
# auditing-consistency (cross-corpus drift audit) — new-audit.sh bootstrap behaviour.
source "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/lib.sh"

test_new_audit_requires_slug() {
  local proj; proj=$(make_tmp_project)
  install_kit_into "$proj"
  if ( cd "$proj" && bash talaka/skills/auditing-consistency/new-audit.sh ) >/dev/null 2>&1; then
...
test_new_audit_creates_folder() {
  local proj; proj=$(make_tmp_project)
  install_kit_into "$proj"
  ( cd "$proj" && bash talaka/skills/auditing-consistency/new-audit.sh model-naming ) >/dev/null
...
test_new_audit_is_idempotent() {
  local proj; proj=$(make_tmp_project)
  install_kit_into "$proj"
  ( cd "$proj" && bash talaka/skills/auditing-consistency/new-audit.sh paths ) >/dev/null
  local dir="$proj/.tlk/audits/$(date +%Y-%m-%d)-paths"
  echo "USER-EDIT" >> "$dir/audit.md"

  local out
  out=$( cd "$proj" && bash talaka/skills/auditing-consistency/new-audit.sh paths )
...
test_new_audit_emits_audit_path() {
  ...
  install_kit_into "$proj"
  local out
  out=$( cd "$proj" && bash talaka/skills/auditing-consistency/new-audit.sh post-rename )
