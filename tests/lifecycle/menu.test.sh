...
_proj() {
  ...
  cp "$KIT_ROOT/kit.sh" "$proj/agentic-kit/"
  printf '%s' "$proj"
}
_akt() { bash "$1/agentic-kit/kit.sh" "${@:2}"; }

test_list_json_is_valid_and_has_components() {
  local proj; proj=$(_proj)
  local out; out=$(_akt "$proj" --list-json 2>&1) || fail "--list-json exited non-zero"
...
test_help_lists_components() {
  local proj; proj=$(_proj)
  local out; out=$(_akt "$proj" --help 2>&1)
...
test_components_menu_requires_tty() {
  ...
  _akt "$proj" components </dev/null >/dev/null 2>&1 && fail "components without TTY/stage should exit non-zero" || true
}
...
test_component_scripts_exist() {
  ...
  assert_file_exists "$KIT_ROOT/tools/install-statusline.sh" "statusline installer exists"
  assert_file_exists "$KIT_ROOT/autoresearch/run.sh"          "autoresearch runner exists"
  assert_file_exists "$KIT_ROOT/tools/memory-hook.sh"         "memory-hook script exists"
}
