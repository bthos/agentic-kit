...
- `/auditing-consistency` — audit the whole instruction/config/doc corpus.
- `/auditing-consistency <glob|dir>` — scope to a subset (e.g. `agents/*.md`, `*/tools/*.sh`).
- After a rename or refactor that touched many files, or when you suspect a value/term has drifted across the codebase.

## Bootstrap

```bash
.claude/skills/auditing-consistency/new-audit.sh <slug>
```
