...
This is the workflow that turns inspiration into a feature. It is **design-only**, like the kit's other design skills: it produces the brief and the adaptation design, then hands the build to `/planning-architecture` → `@bagnik` → `@cmok`.

## When to Use

- `/adapting-patterns <url|repo|gist>` — research and adapt a named source.
- `/adapting-patterns "<technique>"` — adapt a known technique with no single canonical source.
...
```bash
.claude/skills/adapting-patterns/new-adaptation.sh <slug>
```
...
talaka/memory/tools/session.sh agent adapting-patterns
```
...
The build is **not** yours. Append to `handoff-log.md` and hand off to `/planning-architecture` (architecture + tests), citing both artifacts. From there the normal flow applies: planning-architecture → `@bagnik` (test gate) → `@cmok` (build) → `@bagnik` (code QA) → `@zlydni`.
