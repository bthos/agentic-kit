...
- `/curating-knowledge ingest <file | url | pasted text>` — bring a new source into the wiki.
- `/curating-knowledge query "<question>"` — answer from the wiki, with citations.
- `/curating-knowledge lint` — health check; fix the bookkeeping rot.
- `/curating-knowledge` with no arguments — report wiki status: page count, last ingest, last lint, orphan estimate.
...
```bash
.claude/skills/curating-knowledge/new-wiki.sh
```
