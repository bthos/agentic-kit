...
7. **Manifest integrity.** After every accepted mutation, `.agentic-kit.files` must record the new SHA-256 for the changed file. `teardown.sh` must still recognise the file as kit-managed.
...
- `PROJECT.md`, `CLAUDE.md`, `AGENTS.md`, `PIPELINE.md.template`.
