...
This file is the **ontology** for `.artefacts/memory/`. Every entry across the L2/L3 layers follows this shape so `memory-search.sh`, `memory-promote.sh`, and the agents that read memory can rely on a stable contract.
...
| **L1 — Hot State** | `.artefacts/SESSION-STATE.md` | Current active feature, current agent, in-flight decisions. Reset / pruned aggressively. |
| **L2 — Daily Memory** | `.artefacts/memory/YYYY-MM-DD.md` | Per-day rolling log of observations, lessons, decisions. Append-only; rolled over by `memory-rollover.sh`. |
| **L3 — Long-term structured** | `.artefacts/memory/{preferences,system,projects,decisions}.md` | Curated, deduplicated facts grouped by entity type. |
| **L4 — Root summary** | `.artefacts/MEMORY.md` | ≤2 KB index pointing to L3 sections. Read first by every skill. |
| **L5 — Semantic recall** | `.artefacts/memory/index.jsonl` | Chunked text + lightweight retrieval surface for `memory-search.sh`. Auto-rebuilt. |
...
- **curated → hardened** — when an L3 entry has `confidence: high` AND has been referenced in 3+ subsequent features without contradiction, `memory-promote.sh --propose-hardening` writes a patch to `.artefacts/proposed-patches/<agent>.md` for review with `tools/apply-patches.sh`.
...
`.artefacts/MEMORY.md` is **regenerated** by `memory-promote.sh` on every promote run. Hand-edits will be overwritten — capture intent in L3 instead.
...
1. **Read first.** Before any task, read `.artefacts/MEMORY.md`. Drill into L3 only when needed.
