...
The kit is **minimally invasive**: every kit-touched path is either inside `.artefacts/`, the IDE folder (`.claude/`, `.cursor/`, `.github/`), or wrapped in a removable `<!-- agentic-kit:start --> … <!-- agentic-kit:end -->` block. `teardown.sh` strips the block (or removes the file when its SHA-256 still matches the kit copy recorded in `.artefacts/.agentic-kit.files`), so manual edits are always preserved.
...
`--non-interactive` / `-n` is the recommended flag for agents and CI (aliases: `--yes`, `-y`): it skips existing files, suppresses all Y/n prompts, and prints a structured **`[AGENT ACTION REQUIRED]`** block instructing the calling agent to fill `.artefacts/PROJECT.md` itself — no nested CLI process is spawned. The agent reads the script output and uses its own tools (Read / Glob / Edit) to replace the placeholders.

Then open **`.artefacts/PROJECT.md`** and fill in the **Project-Specific Configuration** section:
...
├── .artefacts/                   ← all kit-managed project state lives here
...
`init.sh` appends **one** contiguous block delimited by `# >>> agentic-kit (managed) >>>` … `# <<< agentic-kit (managed) <<<`. Inside it, comments separate **(a) runtime / ephemeral** paths from **(b) per-machine bookkeeping** (`.artefacts/.agentic-kit.cfg` and `.artefacts/.agentic-kit.files`). `teardown.sh` strips the **whole** block in one pass — you do not maintain two independent ignore sections.

**Do not** ignore the entire `.artefacts/` directory unless you also add negation rules so `PIPELINE.md` and `PROJECT.md` stay tracked — otherwise teammates never see the shared pipeline copy. The managed block lists only ephemeral paths plus the optional commented “ignore everything except PIPELINE/PROJECT” recipe for advanced setups.

## What `init.sh` does

**Always (all IDE modes):**

1. Creates `.artefacts/` and copies the canonical pipeline doc + project config:
   - `.artefacts/PIPELINE.md` ← from `agentic-kit/templates/PIPELINE.md.template` (kit-managed; refreshed on `--force`)
   - `.artefacts/PROJECT.md` ← from `agentic-kit/templates/PROJECT.md.template` (your edits preserved unless `--force`)
2. Adds the managed `.gitignore` block described above — ephemeral directories and files under `.artefacts/`, plus `.artefacts/.agentic-kit.cfg` and `.artefacts/.agentic-kit.files`. **`PIPELINE.md` and `PROJECT.md` are not listed** so teams can commit them as usual.
...
**`.artefacts/.agentic-kit.files`** records SHA-256 per kit-managed path (paths are still relative to the **project root**, e.g. `.claude/agents/bagnik.md`). It sits beside `.artefacts/.agentic-kit.cfg` and is listed in the managed `.gitignore` block so it stays local to each checkout.
...
4. Copies `agents/*.md` → `.claude/agents/` (records SHA-256 in **`.artefacts/.agentic-kit.files`**)
...
- New agents and skills — `init.sh` installs missing paths; existing files prompt (or follow **`--skip`** / **`--overwrite-all`**) and refresh hashes in **`.artefacts/.agentic-kit.files`** when overwritten
- Scripts under `agentic-kit/tools/` — they ship with the submodule; `git submodule update` brings new versions
- `.artefacts/PIPELINE.md` — refreshed in place when you pass `--force` (or answer **o**); `update.sh` warns you if `agentic-kit/templates/PIPELINE.md.template` has changed since last init so you know when a refresh is worth running
...
- `.artefacts/PROJECT.md` — project-specific config, never touched (use `--force` to reset from the template)
...
Edit the installed copy under **`.claude/agents/`**, **`.claude/skills/`**, **`.cursor/skills/`**, etc. Once the file content differs from the last kit-installed bytes, its SHA-256 no longer matches **`.artefacts/.agentic-kit.files`**, so **`teardown.sh` leaves it in place** (treats it as manually edited).
...
# managed .gitignore block, remove .artefacts/PIPELINE.md.
agentic-kit/tools/teardown.sh

# Same plus: delete .artefacts/PROJECT.md (after y/N confirmation)
# and rmdir .artefacts/ if empty.
...
- **Agent / skill copies** under `.claude/`, `.cursor/`, `.github/` — deleted only when the on-disk SHA-256 still matches the value recorded in `.artefacts/.agentic-kit.files`. Files you edited by hand are left alone (the script reports them as "modified locally").
- **`.artefacts/PIPELINE.md`** — same SHA-256 check.
- **`.artefacts/PROJECT.md`** — kept by default (it has your project config); removed only with `--full-clean` (and only after a y/N prompt unless `--yes` is passed).
- **`.artefacts/{memory,features,archive,proposed-patches}/`** — never touched by teardown. They are your project's runtime state.
...
|-------|--------------|---------|
| **1. Probe** | `tools/probe-project.sh` writes `.artefacts/PROJECT_PROFILE.md` (stack, frameworks, test/build commands, conventions). All skills read it before starting. | `agentic-kit/tools/init.sh --tune` (or run `probe-project.sh` directly) |
...
> **Override the artefacts directory.** Every memory / autoresearch script honours `ARTEFACTS_DIR` (e.g. `ARTEFACTS_DIR=.kit-state agentic-kit/tools/memory-search.sh "auth"`). The default is `.artefacts`, which `init.sh` records in `.artefacts/.agentic-kit.cfg` as `ARTEFACTS_DIR=…` for drift detection and tooling.
...
**Override the judge model** in `.artefacts/PROJECT.md`:
...
Each skill bundles its own script. Shared scripts live in `agentic-kit/tools/`. Run them from the **project root** so paths like `.artefacts/PROJECT.md` resolve correctly.
...
| Script | What it does |
|--------|-------------|
| `agentic-kit/tools/bump-version.sh patch\|minor` | Bumps version in all files listed in `.artefacts/PROJECT.md` (Cmok uses `patch`, Zlydni uses `minor`) — run from project root |
| `agentic-kit/tools/validate-config.sh` | Checks `.artefacts/PROJECT.md` for unfilled `<placeholder>` values — run after `init.sh` |
...
| `tools/init.sh` | IDE choice: Claude Code, Cursor, Copilot, or all. Sets up `.artefacts/`; copies agents/skills; generates Cursor/Copilot outputs; manages include blocks in `CLAUDE.md` / `AGENTS.md` / `.github/copilot-instructions.md`; manages the `.gitignore` block; maintains **`.artefacts/.agentic-kit.files`**. |
...
| `tools/teardown.sh` | Strips managed include blocks; strips the managed `.gitignore` block; removes kit-installed copies when SHA-256 matches **`.artefacts/.agentic-kit.files`**; removes legacy symlinks and `.cursor/rules/*.mdc`; `--full-clean` also removes `.artefacts/PROJECT.md` and `.artefacts/.agentic-kit.cfg`; `--remove-submodule` deinits git. |
...
See `.artefacts/PIPELINE.md` (Handoff Protocol section) — referenced from `CLAUDE.md`, `AGENTS.md`, and `.github/copilot-instructions.md` via the managed include block — for the full structured handoff format, handoff map, and agent-specific checklists.
...
git add .artefacts/PIPELINE.md .artefacts/PROJECT.md
...
# (.artefacts/.agentic-kit.cfg and .agentic-kit.files are gitignored — do not commit)
git commit -m "chore: add agentic-kit submodule"
```

The managed `.gitignore` block excludes ephemeral tree paths and **`.artefacts/.agentic-kit.files`** / **`.artefacts/.agentic-kit.cfg`** so those stay local to each developer.
